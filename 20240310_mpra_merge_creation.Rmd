...
# mpra_out<-read.delim("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Primary_Data/20240310_analysis/results/OLJR.A_Jurkat_emVAR_glm_20240310.out",header=T, stringsAsFactors = F,sep="\t")
#mpra_out<-read.delim("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Primary_Data/20240310_analysis/results/OLJR.D_Jurkat.Stim_emVAR_glm_20240310.out",header=T, stringsAsFactors = F,sep="\t")
head(mpra_out)
...
```{r}
nrow(mpra_out) # 18326 (tcell, unstim jurkat)
mpra_out <- subset(mpra_out, A_Ctrl_Mean>=20 & B_Ctrl_Mean>=20)
nrow(mpra_out) # 18182(tcell), 18070(unstim jurkat), 18227 (stim jurkat)
```
...
```{r}
ananastra.data.for.mpra <- read.table("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/data/tf_columns_mpra_merge/ananastra_data_for_mpra_hg19.txt", sep="\t", header=T)
...
```{r}
motifbreakr.data.for.mpra <- read.table("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/data/tf_columns_mpra_merge/motifbreakr_data_for_mpra_hg19.txt", sep="\t", header=T)
...
nrow(subset_mpra)
# emVars 545 (tcell glm)  (547 before minimum plasmid cutoff)
# emVars 218 (unstim_jurkat glm)
# emVars 229 (stim_jurkat glm)
subset_mpra2 <- subset(mpra, mpra_sig == "Enhancer_nSkew")
nrow(subset_mpra2)
# pCREs 1125 (tcell glm) (1128 before minimum plasmid cutoff)
# pCREs 8982 (unstim_jurkat glm)
# pCREs 225 (stim_jurkat glm)
subset_mpra3 <- subset(mpra, mpra_sig == "nEnhancer_nSkew")
nrow(subset_mpra3)
# no activity 16471 (tcell glm) (16604 before minimum plasmid cutoff)
# no activity 8829 (unstim_jurkat glm)
# no activity 17732 (stim_jurkat glm)
```
