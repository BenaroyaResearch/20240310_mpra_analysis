...
write.table(dat.biobank, paste0("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/UK_biobank_finemapping_enrichment/data/","uk_biobank_top_pip_mpra_data"), row.names=FALSE,col.names=TRUE,sep="\t")
...
write.table(dat.enrichment, paste0(data.dir,"/",plots_id,"_mpra_uk_biobank_enrichment_plot_dhs_loci_only_table"), row.names = F, col.names = T, sep="\t", quote=F)   
...
# Set up heat map for all the diseases
  ...
  uk.biobank.heatmap.long.form.data <- data.frame(pip=rep(c(0.01, 0.05, 0.1,0.15,0.2),times=82),trait=rep(trait_list,each=5))
