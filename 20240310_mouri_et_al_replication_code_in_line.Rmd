...
```{r}
# Load the MPRAs
tcell<-read.delim(paste0("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/tcell/glm/data/20240310_tcell_glm_mpra_merge_hg38.txt"), header=T, stringsAsFactors = F, sep="\t")
tcell$skew <- ifelse(tcell$mpra_sig == "Enhancer_Skew", "emVar", "no_skew")
tcell <- subset(tcell, select=c(SNP,LogSkew,Skew.logFDR,skew))
names(tcell) <- c("SNP","tcell_LogSkew","tcell_Skew.logFDR","tcell_skew")
jurkat<-read.delim(paste0("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data/20240310_unstim_jurkat_glm_mpra_merge_hg38.txt"), header=T, stringsAsFactors = F, sep="\t")
jurkat$skew <- ifelse(jurkat$mpra_sig == "Enhancer_Skew", "emVar", "no_skew")
jurkat <- subset(jurkat, select=c(SNP,LogSkew,Skew.logFDR,skew))
names(jurkat) <- c("SNP","jurkat_LogSkew","jurkat_Skew.logFDR","jurkat_skew")
# Merge the MPRAs
double_mpra_scatter <- merge(tcell, jurkat, by="SNP")

# Allelic Bias plot
Allelic_bias_scatter_plot <- ggplot(data=double_mpra_scatter, aes(x=tcell_LogSkew, y=jurkat_LogSkew))+
  geom_point(data=subset(double_mpra_scatter, jurkat_skew!="emVar" & tcell_skew!="emVar"),color="gray90",size=2) + 
  geom_point(data=subset(double_mpra_scatter, jurkat_skew=="emVar"),color="#5D93CD",size=2) + 
  geom_point(data=subset(double_mpra_scatter, tcell_skew=="emVar"),color="#E14A4A",size=2) + 
  geom_point(data=subset(double_mpra_scatter, tcell_skew=="emVar" & jurkat_skew=="emVar"),color="#e14ae1",size=2) + 
  labs(title ="Unstimulated Jurkat and Primary Tcell MPRA allelic Bias Comparsion", x ="Primary Tcell Allelic Bias", y = "Jurkat Allelic Bias") +
  theme_bw() 
...
plots_id <- "20240310_mpra_compare_" 
# creating the plot name from the plots_id
plots_name <- paste0(plots_id,"_allelic_bias_scatter_plot",".pdf")
# Saving the plot in the plots directory 
ggsave(plots_name,plot = Allelic_bias_scatter_plot, path = plots.dir, width = 12, height = 8)


cor.test(x=double_mpra_scatter$tcell_LogSkew,y=double_mpra_scatter$jurkat_LogSkew, alternative = c("two.sided"), method = c("pearson"), conf.level = 0.95)
```
...
mpra_merge <- read.delim(paste0(mpra.dir), header=T, stringsAsFactors = F, sep="\t")
...
data.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/tcell/glm/data"
# plots_id is the prefix to the plot. It should end with an underscore to look nice
plots_id <- "20240310_tcell_glm_hg38" 
cell_type <- "tcell"

#mpra.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data/20240310_unstim_jurkat_glm_mpra_merge_hg38.txt"
#mpra_merge <- read.delim(paste0(mpra.dir), header=T, stringsAsFactors = F, sep="\t")
    # An input in every function is going to be a subtitle which appears on all of the graphs to indicate which analysis you are preforming. This subtitle is a description in quotes and here we make it an object so we can put it in all of the plots. There will also be inputs for the plots directory and the plots identifier. 
#subtitle <- "unstim_jurkat glm 03/10/24"
    # plots.dir is the place where your plots are stored 
#plots.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/plots"
    # This is where the data is stored
#data.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data"
    # plots_id is the prefix to the plot. It should end with an underscore to look nice
#plots_id <- "20240310_unstim_jurkat_glm_hg38" 
#cell_type <- "jurkat"
```
...
# Give the data frame the proper headings
  colnames(df) <- c(paste0(subtitle, "MPRA"))
  gtable_compare <- gt(df, rownames_to_stub = TRUE)
  gtable_compare
...
# write.table(lead.snp.dat, paste0(data.dir,"/",plots_id,"_how_many_emvars_per_loci.txt"), row.names = F, col.names = T, sep="\t", quote=F) 
...
write.table(lead.snp.dat.all, paste0(data.dir,"/",plots_id,"_how_many_variants_per_loci_all_types.txt"), row.names = F, col.names = T, sep="\t", quote=F) 
```
...
#mpra.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data/20240310_unstim_jurkat_glm_mpra_merge_hg19.txt"
#mpra_merge <- read.delim(paste0(mpra.dir), header=T, stringsAsFactors = F, sep="\t")
    # An input in every function is going to be a subtitle which appears on all of the graphs to indicate which analysis you are preforming. This subtitle is a description in quotes and here we make it an object so we can put it in all of the plots. There will also be inputs for the plots directory and the plots identifier. 
#subtitle <- "Unstim Jurkat glm 03/10/24"
    # plots.dir is the place where your plots are stored 
#plots.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/plots"
    # This is where the data is stored
#data.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data"
    # plots_id is the prefix to the plot. It should end with an underscore to look nice
#plots_id <- "20240310_unstim_jurkat_glm_hg19" 
#cell_type <- "jurkat"

lead.snp.dat.all <- data.frame(table(mpra_merge$lead_snp))
names(lead.snp.dat.all) <- c("lead_snp", "number_of_variants")
lead.snp.dat.all<-lead.snp.dat.all[order(-lead.snp.dat.all$number_of_variants),]
```
...
write.table(dat.enrichment, paste0(data.dir,"/",plots_id,"_mpra_pics_enrichment_plot_all_loci_table.txt"), row.names = F, col.names = T, sep="\t", quote=F)  
...
write.table(dat.enrichment, paste0(data.dir,"/",plots_id,"_mpra_pics_enrichment_plot_dhs_loci_only_table.txt"), row.names = F, col.names = T, sep="\t", quote=F)
...
write.table(dat.sens_spec, paste0(data.dir,"/",plots_id,"_dhsloci_",dhs_loci,"_","sensitivity_and_specificity"), row.names = F, col.names = T, sep="\t", quote=F)
...
mpra <- mpra_merge
# This is 
dat.skew <-data.frame(read_excel("/nfs/jray/screens/Mouri_et_al_MPRA/asc/41588_2019_505_MOESM3_ESM.xlsx", sheet=10), stringsAsFactors = F)
...
write.table(dat.enrichment, paste0(data.dir,"/", plots_id, "_histone_cage_dhs_data.txt"), row.names=FALSE,col.names=TRUE,sep="\t", quote=FALSE)
...
write.table(dat.enrichment, paste0(data.dir,"/", plots_id, "_chrommhmm_histone_data.txt"), row.names=FALSE,col.names=TRUE,sep="\t", quote=FALSE)
...
mpra<-read.delim(paste0(mpra.dir), header=T, stringsAsFactors = F, sep="\t")
# motifbreakr.bed.dir 
motifbreakr.bed.dir <- paste0("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data/20240310_unstim_jurkat_glm_hg19_mpra_motifbreakr.bed")
# motifbreakr.results.dir
motifbreakr.results.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data/20240310_unstim_jurkat_glm_hg19_mpra_motifbreakr_results.txt"
...
motifbreakr.bed.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/unstim_jurkat/glm/data/20240310_unstim_jurkat_glm_hg19_mpra_motifbreakr.bed"
